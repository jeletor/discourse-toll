...
class WalletProvider {
  ...
  async createInvoice(sats, description) {
    ...
    // Payment hash must come from the NWC response — never derive it ourselves
    const paymentHash = result.payment_hash || result.paymentHash;
    if (!paymentHash) {
      throw new Error('NWC response missing payment_hash');
    }
  ...
  verifyPreimage(preimage, paymentHash) {
    // Validate hex format: must be exactly 32 bytes (64 hex chars)
